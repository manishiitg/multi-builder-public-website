...
Pages: `/` (home), `/product/`, `/agents/`, `/pricing/`, `/enterprise/`, five enterprise use-case pages under `/enterprise/<slug>/` (data in `scripts/launch/uc_data.py`, which also drives the header's Enterprise dropdown), and `/404.html` (all generated). `/how/`, `/use-cases/` and `/updates/` are retired and redirect via `_redirects`. Social images: `NODE_PATH=<playwright node_modules> python3 scripts/launch/og.py`. Hero animation: HyperFrames source in `scripts/launch/hero-animation-src/`, outputs in `assets/hero/goal-loop-*`. Public copy must not say "Pulse" (use "Auto-improve") and must not use real numbers from our own workspace; examples are illustrative.
