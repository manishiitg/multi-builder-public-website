...
PRIVACY=[
 ...
 ('The website', '<p>The website does not use advertising or tracking cookies and has no sign-up forms. We use Cloudflare Web Analytics, which measures page views without cookies and without building a profile of you. Our hosting provider processes standard request data (such as IP address and browser type) to serve and protect the site.</p>'),
...
<p>We do not sell personal data, and we do not use your workspace data to train AI models.</p>'''),
 ...
 ('Other service providers', '<p>We use a small number of providers to run the service, such as cloud hosting, email delivery, payment processing and the connectors you choose to enable (for example Slack, WhatsApp or Gmail). They process data only as needed to provide their part of the service.</p>'),
